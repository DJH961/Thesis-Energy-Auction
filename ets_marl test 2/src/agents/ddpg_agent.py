...
class DDPGAgent:
    ...
    def __init__(
        ...
        # --- Exploration noise (per-dimension sigma as fraction of action range) ---
        action_ranges = action_high - action_low          # e.g. [130, 5, 0.03, 1.5, 10]
        # sigma_i = ou_sigma * range_i → each dim gets noise proportional to its scale
        # ou_sigma acts as a fraction: 0.10 = 10% of range
        sigma_scale = action_ranges                        # raw ranges as scale

        self.noise = OUNoise(
            action_dim=action_dim,
            theta=ddpg["ou_theta"],
            mu=ddpg["ou_mu"],
            sigma=ddpg["ou_sigma"],            # now acts as fraction of range
            seed=seed,
            sigma_scale=sigma_scale,
        )

        # Noise decay schedule
        self.epsilon_start = ddpg.get("epsilon_start", 1.0)
        self.epsilon_end   = ddpg.get("epsilon_end", 0.05)
        self.epsilon_decay_episodes = ddpg.get("epsilon_decay_episodes", 8000)

        # Gradient clipping (configurable)
        self.grad_clip = ddpg.get("grad_clip", 1.0)
    ...
    def update(self) -> Optional[dict]:
        ...
        # NOTE: reward is already normalised in _compute_rewards (/300 scale).
        # Per-batch normalisation was removed because it makes Q-targets
        # inconsistent across batches, preventing Critic convergence.
    ...
    def decay_noise(self, episode: int):
        """Linearly decay exploration noise from epsilon_start to epsilon_end."""
