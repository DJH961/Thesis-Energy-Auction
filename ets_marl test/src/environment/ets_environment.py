...
One step = one year:
  ...
  3. Auction: agents submit (price_markup, quantity) → uniform-price clearing
...
Action space per agent: 5-dimensional
    [0] price_markup   : markup on expected price, range [-0.30, +0.30]
...
class ETSEnvironment(gym.Env):
    ...
    def step(self, actions: np.ndarray):
        ...
        actions : np.ndarray, shape (N_agents, 5)
            Each row: [price_markup, quantity_bid, delta_green,
        ...
        tnac = max(0.0, total_estimate - cap_t)
        ...
        # action[:,0] = markup in [-0.30, +0.30]
        # bid_price = expected_price * (1 + markup), clipped to [1, 200]
        # ----------------------------------------------------------------
        auction_actions = actions[:, :2].copy()
        auction_actions[:, 0] = self.expected_price * (1.0 + actions[:, 0])
        auction_actions[:, 0] = np.clip(auction_actions[:, 0], 1.0, 200.0)
        ...
        # ----------------------------------------------------------------
        # 9. Advance year — update expected price (moving average last 5)
        # ----------------------------------------------------------------
        self._price_history.append(clearing_price)
        if len(self._price_history) > 5:
            self._price_history.pop(0)
        valid = [p for p in self._price_history if p > 0]
        if valid:
            self.expected_price = max(
                float(np.mean(valid)),
                self.config["price"].get("price_floor", 45.5)
            )
    ...
    def _compute_rewards(self, payments, trade_costs, penalties, switching_costs):
        """
        reward_i = -(w_cost * total_cost_norm + w_green * fossil_frac)

        total_cost includes:
          - auction payments (always positive, company pays)
          - trade_costs on secondary market:
              POSITIVE = company bought (paid money)
              NEGATIVE = company sold (received money) → reduces total_cost
          - penalties (always positive)
          - switching costs (always positive)

        Note: abs() is NOT applied to trade_costs — selling credits is a gain.
        """
        rewards = np.zeros(self.n_agents)
        for i, company in enumerate(self.companies):
            total_cost = (
                payments[i] / 1e6        # auction payment (M€)
                + trade_costs[i]         # secondary market net cost (M€), can be negative
                + penalties[i] / 1e6     # penalty (M€)
                + switching_costs[i]     # green investment cost (M€)
            )
            green_component = company.fossil_frac  # 0 = fully green, 1 = fully fossil

            rewards[i] = -(
                company.w_cost * (total_cost / 10.0)
    ...
    def _build_spaces(self):
        ...
        # Action per agent: [price_markup, qty_bid, delta_green,
        #                     secondary_price_mult, secondary_qty]
        act_low = np.array([
            aq.get("price_min", -0.30),   # markup minimo
            0.0,                           # qty_bid min
            0.0,                           # delta_green min
