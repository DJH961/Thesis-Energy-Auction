...
- **ESG signal** — saved-carbon hybrid: `esg_scale × (stock_w × ef_ratio + flow_w × ef_ratio × (anchor_real_t / anchor_real_0) + speed_coef × max(0, Δgreen)) × compliance_gate`. The stock term pays sustained green share every year; the flow term monetises avoided carbon at the live anchor (the social shadow price); the speed bonus rewards motion this year. `esg.scale` is calibrated for a ~50/50 financial-vs-ESG split for balanced (`[w_cost, w_green] = [0.5, 0.5]`) agents — see `docs/esg_reward_design.md` for the design, balance sweep, and stress scenarios.
...
- **Joint budget gate**: bid quantity is sized against expected settlement cost (`safety_mult × max(reserve, MA3_inflated, anchor) + collateral(bid_p) per Mt`), not against `bid_p × bid_q` — in a uniform-price auction the agent pays the clearing, not its bid. Acts as a last line of defence: only fires when expected settlement exceeds the gate's cash buffer (operating + a fraction of treasury). Treasury is partial by default (`treasury_fraction = 0.5`) — it's stress-reserve for genuine price spikes, not a routine sizing buffer; emergency-loan headroom is also not included by default (`include_loan_headroom = false`). When the bid would over-spend cash, a two-stage protocol runs: shrink `bid_q` toward `need`, then reduce `bid_p` toward `max(reserve, MA3_inflated)`, then last-resort shrink `bid_q` below need. The `need` floor uses the deterministic estimate the agent saw at obs time by default (fair — only clip on info available at bid time); set `shock_aware_need_floor = true` to use the realised shock instead. Knobs: `auction.budget_gate.{enabled, safety_mult, notional_safety_mult, protect_need_floor, inflation_aware_ma3, treasury_fraction, treasury_max_abs, include_loan_headroom, shock_aware_need_floor}`.
...
- **Budget headroom observation**: Phase-1 observation reports current annual budget headroom (`1.0` fresh, `0.0` at limit, negative on overspend), plus separate dims for bid-price clip, budget-gate price clip, qty clip ratio, invest-clip ratio, and a forward-looking `compliance_affordability = (need × expected_clearing) / cash` (so agents don't have to learn affordability through repeated clip events).
