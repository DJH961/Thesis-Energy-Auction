...
- **Joint budget gate**: bid quantity is sized against expected settlement cost (`safety_mult × max(reserve, MA3_inflated, anchor) + collateral(bid_p) per Mt`), not against `bid_p × bid_q` — in a uniform-price auction the agent pays the clearing, not its bid. When the bid would over-spend cash, a two-stage protocol runs: shrink `bid_q` toward `need`, then reduce `bid_p` toward `max(reserve, MA3_inflated)`, then last-resort shrink `bid_q` below need. The cash buffer includes operating + treasury + emergency-loan headroom, and the `need` floor incorporates the realised emission shock so a positive shock cannot push qty below the actual obligation. Knobs: `auction.budget_gate.{enabled, safety_mult, notional_safety_mult, protect_need_floor, inflation_aware_ma3}`.
