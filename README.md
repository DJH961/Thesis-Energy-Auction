...
- **Joint budget gate**: bid quantity is sized against expected settlement cost (`safety_mult × max(reserve, MA3_inflated, anchor) + collateral(bid_p) per Mt`), not against `bid_p × bid_q` — in a uniform-price auction the agent pays the clearing, not its bid. Acts as a last line of defence: only fires when expected settlement exceeds ordinary cash (operating + treasury). Emergency-loan headroom is **not** included by default — the loan is a settlement-time safety net, not a sizing tool — but can be opted into via `include_loan_headroom`. When the bid would over-spend cash, a two-stage protocol runs: shrink `bid_q` toward `need`, then reduce `bid_p` toward `max(reserve, MA3_inflated)`, then last-resort shrink `bid_q` below need. The `need` floor uses the realised emission shock (which the env knows from upstream draw, even though the agent did not see it at bid-time) so a positive shock cannot push qty below the actual obligation. Knobs: `auction.budget_gate.{enabled, safety_mult, notional_safety_mult, protect_need_floor, inflation_aware_ma3, include_loan_headroom, shock_aware_need_floor}`.
