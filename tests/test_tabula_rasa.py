...
def test_tabula_rasa_uniform_exploration():
    ...
    obs1 = np.zeros(36, dtype=np.float32)
    obs1[3] = 0.156  # expected_price obs (not used as anchor in WTP mode)
    obs1[10] = 0.3   # estimated_need / 10 = 3 Mt
    obs1[27] = 0.8   # budget_headroom = 80%
    np.random.seed(42)
    ...
    # WTP anchor = min(MAC + 0.5*(penalty - MAC), budget_avail / need)
    # = min(48 + 0.5*(138.75 - 48), 0.8*880 / 3) = min(93.375, 234.67) = 93.375
    wtp_anchor = 93.375
    under = sum(p < wtp_anchor for p in prices)
    over = sum(p > wtp_anchor for p in prices)
    ...
    # Uniform mode keeps side-balanced bid-price sampling around WTP anchor,
