...
def test_epsilon_greedy_auction(sample_config):
    ...
    # Use a controlled obs with deterministic values.
    # obs[10] = need (normalized), obs[3] = expected_price.
    # Set need small so budget constraint doesn't dominate WTP.
    # WTP anchor ≈ MAC + 0.5*(penalty - MAC) = 48 + 0.5*(138.75 - 48) ≈ 93.4
    np.random.seed(42)
    obs = np.random.randn(21).astype(np.float32)
    obs[3] = 0.65
    obs[10] = 0.1  # small need → budget not binding → WTP ≈ 93
    bid_prices = []
    for _ in range(200):
    ...
    # Anchored mode: WTP anchor ≈ 93 (MAC + half gap to penalty), not obs[3]*price_max
    wtp_anchor = 48.0 + 0.5 * (138.75 - 48.0)  # ≈ 93.375
    # (a) All within bounds
    ...
    # (c) Mean is near WTP anchor, not the midpoint (80)
...
# ---------------------------------------------------------------------------
# Test 9: Secondary buy price shifts WTP anchor (Approach C+D)
# ---------------------------------------------------------------------------

def test_secondary_buy_price_shifts_exploration(sample_config):
    """Approach C+D: when last_secondary_buy_price > WTP economic, the
    exploration anchor shifts upward, producing higher mean bids."""
    from src.agents.ppo_agent import PPOAgent
    ...
    obs[3] = 0.65   # expected price
    obs[10] = 0.05   # small need → budget not binding

    # Baseline: no secondary buy price
    bids_baseline = []
    for _ in range(300):
        action, _, _ = agent.select_auction_action(obs, epsilon=1.0,
                                                   last_secondary_buy_price=0.0)
        bids_baseline.append(action[0])

    # With secondary buy price of 110 EUR (> WTP economic ≈ 93)
    bids_shifted = []
    for _ in range(300):
        action, _, _ = agent.select_auction_action(obs, epsilon=1.0,
                                                   last_secondary_buy_price=110.0)
        bids_shifted.append(action[0])

    mean_baseline = np.mean(bids_baseline)
    mean_shifted = np.mean(bids_shifted)

    # The shifted anchor should produce higher mean bids
    assert mean_shifted > mean_baseline, (
        f"Secondary buy price should shift bids up: baseline={mean_baseline:.1f}, "
        f"shifted={mean_shifted:.1f}")
    # Shifted mean should be closer to the blended anchor:
    # blend = 0.5 * 110 + 0.5 * 93.375 ≈ 101.7
    assert mean_shifted > 90.0, (
        f"Shifted mean {mean_shifted:.1f} should be above 90 (blend ≈ 101.7)")
