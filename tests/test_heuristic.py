...
class TestAuctionAction:
    ...
    def test_green_and_financial_bid_same_for_same_state(self, config):
        ...
        Agents 0/1 now carry asymmetric budgets for collusion-breaking, so we
        align their financial state explicitly to isolate reward-weight effect.
        """
        import copy
        cfg = copy.deepcopy(config)
        cfg["budget"]["annual_budgets"][1] = cfg["budget"]["annual_budgets"][0]
        cfg["budget"]["debt_headrooms"][1] = cfg["budget"]["debt_headrooms"][0]
        cfg["budget"]["capex_throughputs"][1] = cfg["budget"]["capex_throughputs"][0]
        c_fin = make_company(cfg, agent_id=0)
        c_grn = make_company(cfg, agent_id=1)
        bank = 1.0
        a_fin = auction_action(c_fin, price_ma3=80.0, current_year=5, n_years=12, config=cfg, bank=bank)
        a_grn = auction_action(c_grn, price_ma3=80.0, current_year=5, n_years=12, config=cfg, bank=bank)
        assert a_grn[0] == pytest.approx(a_fin[0], abs=1e-6)

    def test_year0_avg_bid_in_realistic_band(self, config):
        """At MA3=80 and mixed coverage, year-0 average bid should be in [85, 130] EUR/t."""
        ...
        assert 85.0 <= avg_bid <= 130.0, f"Year-0 average bid out of band: {avg_bid:.2f}"
