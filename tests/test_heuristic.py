...
class TestAuctionAction:
    ...
    def test_year0_avg_bid_in_realistic_band(self, config):
        """At MA3=80 and mixed coverage, year-0 average bid should be in [85, 120] EUR/t.
        With the tighter 45-75% green mix, lower compliance need reduces budget pressure,
        allowing WTP-economic to drive bids (which can reach ~93+ EUR/t at typical urgency)."""
        ...
        assert 85.0 <= avg_bid <= 120.0, f"Year-0 average bid out of band: {avg_bid:.2f}"
