...
def test_static_reserve_equals_config():
    ...
    env = load_env()
    env.config["ets"]["reserve_price_mode"] = "static"
    env.reset(seed=42)
...
def test_heuristic_npv_uses_discounting():
    ...
    # Use an explicit coal-heavy mix so the NPV discounting test remains valid
    # regardless of config's initial_mix (which may be green-leaning).
    coal_heavy_mix = [0.40, 0.40, 0.10, 0.05, 0.05]

    rng = np.random.default_rng(42)
    company = Company(
        agent_id=0,
        config=config_base,
        initial_mix=coal_heavy_mix,
        rng=rng,
    )
