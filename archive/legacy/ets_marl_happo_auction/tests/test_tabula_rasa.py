...
def test_tabula_rasa_uniform_exploration():
    ...
    agent = PPOAgent(
        ...
        obs_dim_phase1=22,
        obs_dim_phase2=29,
        auction_action_low=np.array([30.0, 0.3, 30.0, 0.3, 30.0, 0.3, 0.0, -1, -1, -1], dtype=np.float32),
        auction_action_high=np.array([500.0, 2.0, 500.0, 2.0, 500.0, 2.0, 0.2, 1, 1, 1], dtype=np.float32),
...
def test_tabula_rasa_disabled_no_effect(monkeypatch):
    ...
    assert resolved["exploration"]["auction_anchors"] == [75.0, 0.40, 80.0, 0.40, 85.0, 0.40, 0.03, 0.3, -0.5, 0.5]
