...
**Pure MARL, Reward Simplification, Structural Scarcity, WTP Exploration, PAB Curriculum, Reviewer-Addressed Fixes**

### Overview

v7.11.0 is a major revision covering two rounds of changes:
1. **v7.8→v7.11 core** (reward simplification, market redesign, training schedule)
2. **Post-review fixes** addressing reviewer feedback on items 1.4, 1.5, 1.6, 2.4, 2.6, 2.7, 3.2

See `plan/v7.8_to_v7.11_changelog.md` for the full diff analysis and EU ETS regulatory justifications.

### 1. Market Structure

- **1.1 Pure MARL**: `n_bot_agents: 0` — 8 learning agents, no heuristic bots.
- **1.2 Structural over-subscription**: `cap_overhead_pct: -0.10` — cap 10% below total emissions.
- **1.3 MAC-anchored reserve price**: `reserve_price: 45.0` EUR/t (just below MAC 48 EUR/t).
- **1.4 Qty multiplier bounds**: `[0.5, 2.0]` — wide action space preserving strategic flexibility
  for secondary-market-oriented under-bidding and thin-market over-bidding.
  *(Reviewer fix: was [0.85, 1.5], too restrictive.)*
- **1.5 Unsold volume reschedule**: `unsold_to_msr: false` — unsold lots roll to next auction
  per Article 10(6); only TNAC-triggered surpluses flow to MSR.
  *(Reviewer fix: was true, over-inflated MSR reserve.)*
- **1.6 Carry-forward cap**: `carry_forward_cap: 1.0` (1x annual emissions).
  Real ETS = zero tolerance (Art.16); 1.0 is a simulation compromise for learning stability.
  TODO: ablation `[0, 0.5, 1.0]`. *(Reviewer: needs ablation evidence.)*
- **1.7 Realistic bank seed**: `initial_bank_fraction: 0.25`, seed range `[0.15, 0.35]`.

### 2. Reward Function

- **2.1 Fixed REWARD_SCALE**: 1000 M EUR denominator for all agents (was per-agent budget).
- **2.2 Baseline removal**: No running-mean cost subtraction.
- **2.3 Shaping removal**: `green_bonus`, `efficiency_bonus` removed — carbon price IS the green incentive.
- **2.4 Soft budget penalties removed**: Enforced via `investment_hard_gate` + action clipping.
  Audit confirms all spending paths covered (auction=collateral-clipped, secondary=cash-capped,
  investment=hard-gated+budget-ceiling+capex-throughput). *(Reviewer: requested audit — passed.)*
- **2.5 Clean credit assignment**: Coverage credit + gap closure signals in phase-decomposed rewards.
- **2.6 Terminal bank value**: `log1p(capped_holdings)` — diminishing marginal returns.
  Prevents pure-hoarding equilibria; excess holdings less valuable at margin.
  *(Reviewer fix: was linear, incentivized hoarding.)*
- **2.7 Terminal queue value restored**: NPV of carbon savings with **completion-fraction discount**.
  `value *= years_completed / total_delay`. Projects just started → near-zero value;
  nearly-complete → full value. Prevents last-minute gaming while rewarding legitimate long-lead
  investment (e.g., 7-year offshore wind past episode midpoint).
  *(Reviewer fix: was zeroed out.)*
- **2.8–2.10**: ESG scale simplification, reward channel reduction, `s_financial` clamping fix.

### 3. Exploration & Training

- **3.1 Schedule rebalancing**: `epsilon_start: 0.30` (was 0.50), `epsilon_decay_frac: 0.90`,
  `epsilon_final: 0.10`, `critic_warmup_frac: 0.03` (was 0.10).
- **3.2 Dead shaping config removed**: `shaping_weight_floor: 0.0`, orphaned `shaping_beta`
  and `shaping_gamma` removed from config. `shaping_weight` retained for diagnostic logging only.
  *(Reviewer fix: was 0.10 floor on already-removed signals = dead noise.)*

### 4. New Features (This Session)

- **Option B — Collateral increase**: `collateral_rate: 0.15`, `collateral_fraction: 0.20`.
  Higher bid collateral makes reckless overbidding costlier.
- **Option C — Cover ratio in obs**: `obs[33] = clip(cover_ratio, 0, 3) / 3`.
  Phase-1 obs expanded from 33D to **34D** base + 6*(N-1) opponent dims.
- **Option D — WTP-anchored exploration**: Replaces expected-price anchor with
  `WTP = MAC + 0.5*(penalty - MAC)` ≈ 93 EUR, budget-constrained. Explores around
  economically meaningful anchor instead of MA3 price.
- **Narrower initial mix spread**: 45–75% green, paired financial/ESG agents.
- **PAB pricing curriculum**: `pricing_curriculum.pab_frac: 0.20` — first 20% of episodes
  use pay-as-bid (direct bid-price gradient), then switch to uniform.

### 5. Diagnostics & Infrastructure

- **5.1 CSV snapshot logging**: Every 2500 episodes to `results/snapshots/`.
- **Terminal liquidation values** now correctly sum bank + queue contributions.
- **CLAUDE.md** updated to v7.11.0 with 34D obs dimensions.

### 6. Test Adaptations

- Phase-1 obs assertions: 33→34 across `test_company.py`, `test_environment.py`.
- WTP anchor tests in `test_tabula_rasa.py`: obs expanded to 34D, WTP-relevant dims set.
- Epsilon schedule: 0.80→0.90 decay fraction.
- Bid band widened: `[85, 130]` (was `[85, 100]`).
- NPV discount tests: hardcoded coal-heavy mix `[0.40, 0.40, 0.10, 0.05, 0.05]` to decouple from config.
- Paired agent mixes fixed (A1=A2, A3=A4, etc.).
- All **264 tests pass**.
