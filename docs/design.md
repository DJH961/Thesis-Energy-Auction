...
defaults are `[0, 0, 0.010, 0.006, 0.015]` for coal/gas/onshore/offshore/solar).
A scalar `construction_jitter.p_cancel` is honoured as a fallback for callers
that don't set the per-tech vector.
