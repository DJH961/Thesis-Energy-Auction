...
def test_two_phase_step_runs():
    ...
    auction_actions = np.random.uniform(
        ...
        [200.0, 5.0, 200.0, 5.0, 200.0, 5.0, 0.05, 1.0, 1.0, 1.0],
        size=(n_agents, 10)
    ).astype(np.float32)
...
def test_green_fraction_non_decreasing():
    ...
    for _ in range(env.n_years):
        ...
        # 3-tranche bid: same price, split qty
        auction_actions[:, 0] = 100.0  # p1
        auction_actions[:, 1] = 0.67   # q1
        auction_actions[:, 2] = 100.0  # p2
        auction_actions[:, 3] = 0.67   # q2
        auction_actions[:, 4] = 100.0  # p3
        auction_actions[:, 5] = 0.67   # q3
        auction_actions[:, 6] = 0.03   # invest_frac
...
def test_p8_obs_dims():
    """Phase 1 obs should be 26D base (after Phase G + 2 safety dims) + 5*(N_total-1) opponent dims.
    Phase 2 obs = Phase 1 + 14 (7 standard + 6 D1/D2 + 1 collateral)."""
    env = load_env()
    obs, _ = env.reset()
    n_agents = env.config["companies"]["n_agents"]
    n_total = n_agents + env.config["companies"].get("n_bot_agents", 0)
    opp_enabled = env.config.get("opponent_modeling", {}).get("enabled", False)
    # Phase G+safety: 24 → 26 base dims (+2 new)
    expected_p1 = 26 + (5 * (n_total - 1) if opp_enabled else 0)
    # Phase D+collateral: +6 D1/D2 tranche feedback + 1 collateral (total +14 vs Phase 1)
    expected_p2 = expected_p1 + 14  # 7 standard + 6 D1/D2 + 1 collateral
...
def test_defaulted_volume_not_double_counted_with_unsold_rollover():
    ...
    # Aggressive bids across all tranches to saturate supply and trigger defaults.
    auction_actions = np.zeros((n_agents, 10), dtype=np.float32)
    auction_actions[:, 0] = 500.0
    auction_actions[:, 1] = 2.0
    auction_actions[:, 2] = 500.0
    auction_actions[:, 3] = 2.0
    auction_actions[:, 4] = 500.0
    auction_actions[:, 5] = 2.0
