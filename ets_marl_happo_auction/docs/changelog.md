...
- The full feature set introduced in **v8.5.0** remains in effect (dual-ceiling WTP, marginal-EF revenue budgeting, collateral diagnostics, extended per-agent diagnostics/logging, and compliance smoke tests), now with the compliance-recovery and budget-envelope improvements above.
...
- **Phase 1 obs extended** from 26D to 29D with three new financial-awareness dims:
...
- **Phase 2 obs extended** from +14 to +16 with two compliance-awareness dims:
...
- **Tiered budget penalty**: Replaced 3-tier contingency/quadratic/hard system with
  ...
  overshoot amount, not full budget. Applied to 10D action format / 3-tranche auction bids.
...
- **14 new tests** per codebase covering: reward channel population (4), heuristic
  ...
  reward channels in integration (1). All ported to auction codebase with
  3-tranche action format adaptations.
