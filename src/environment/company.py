...
class Company:
    ...
    # ------------------------------------------------------------------
    # Observations — Phase 1: 34D base (+6*(N-1) opponent) | Phase 2: +10
    # ------------------------------------------------------------------
    ...
    def get_observation_phase1(self, year, cap_t, last_clearing_price,
        ...
        [33] last_cover_ratio: last auction's total_demand / auction_volume (clipped [0,3])
             (>1 = over-subscribed, <1 = under-subscribed; informs bid-price strategy)
        [34] own_last_secondary_buy_price / price_max: what THIS agent paid per Mt
             on the secondary market last year (0 if didn't buy). Enables the agent
             to learn: "I overpaid on secondary → I should bid higher at auction."
        [35] cumulative_coverage_ratio: total_alloc_so_far / total_emissions_so_far
             (clipped [0,2], normalized /2). <1 means the agent is falling behind on
             compliance — gives the policy a direct view of the accumulating deficit.
        ...
        base = np.array([
            ...
            float(np.clip(last_cover_ratio, 0.0, 3.0)) / 3.0,          # [33] last auction cover ratio (normalized)
            float(own_last_secondary_buy_price) / pn,                     # [34] own last secondary buy price (normalized)
            float(np.clip(cumulative_coverage_ratio, 0.0, 2.0)) / 2.0,  # [35] cumulative coverage ratio (normalized)
        ], dtype=np.float32)
