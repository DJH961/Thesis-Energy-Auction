...
class ETSEnvironment(gym.Env):
    ...
    def __init__(self, config: dict, seed: Optional[int] = None):
        ...
        # Per-agent last secondary buy price (for WTP anchor feedback)
    ...
    def step_secondary(self, secondary_actions: np.ndarray):
        ...
        # Update secondary profit EMA and per-agent buy price tracking
        for i in range(self.n_total):
            if not active_mask[i]:
            ...
            if trade_qtys[i] < -1e-6:
                ...
                # Seller: no secondary buy price update (keep previous value)
    ...
    def _compute_rewards(self, payments, trade_costs, penalties,
        ...
        if is_final_year:
            ...
            for i, company in enumerate(self.companies):
                ...
                # Terminal bank value: log1p for diminishing marginal returns,
                # capped at 2× annual need. log1p prevents pure-hoarding equilibria
                # by making excess holdings less valuable at the margin.
                if terminal_bank:
                    annual_need = max(company.compute_estimate_need(), 0.1)
                    capped_holdings = min(self.holdings[i], 2.0 * annual_need)
                    bank_value = float(np.log1p(capped_holdings)) * terminal_price / REWARD_SCALE
        ...
        # --- Terminal queue value (final year only) ---
        terminal_queue = reward_cfg.get("terminal_queue_value", False)
        terminal_queue_values = np.zeros(self.n_total)
        if is_final_year and terminal_queue:
            payoff_years = reward_cfg.get("terminal_payoff_years", 5)
            companies_cfg = self.config.get("companies", {})
            ef = companies_cfg.get("emission_factors", [0.820, 0.490, 0.011, 0.012, 0.048])
            deploy_delays = companies_cfg.get("deploy_delays", [0, 0, 4, 7, 2])
            output_twh = companies_cfg.get("output_twh", 10.0)
            discount_rate = self.config.get("investment", {}).get("discount_rate", 0.05)
            ...
            for i, company in enumerate(self.companies):
                ...
                for item in company._construction_queue:
                    ...
                    # Completion fraction discount: projects further from completion
                    # are worth less, preventing last-minute gaming.
                    total_delay = max(deploy_delays[tech_idx], 1)
                    years_completed = max(0, total_delay - (completion_year - self.current_year))
                    completion_frac = float(years_completed) / float(total_delay)
                    # Carbon savings: switching from current avg fossil EF to green tech
                    avg_fossil_ef = (ef[0] * company.mix[0] + ef[1] * company.mix[1]) / max(
                        company.mix[0] + company.mix[1], 1e-6)
                    carbon_saved_per_year = (avg_fossil_ef - ef[tech_idx]) * frac_delta * output_twh
                    # NPV of carbon savings over payoff_years
                    if discount_rate > 0:
                        annuity = (1 - (1 + discount_rate) ** (-payoff_years)) / discount_rate
                    else:
                        annuity = float(payoff_years)
                    npv = carbon_saved_per_year * terminal_price * annuity
                    queue_value += max(0.0, npv * completion_frac)
                terminal_queue_values[i] = queue_value / REWARD_SCALE
                rewards[i] += terminal_queue_values[i]
                base_rewards[i] += terminal_queue_values[i]
