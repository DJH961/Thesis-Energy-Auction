...
class Company:
    ...
    # ------------------------------------------------------------------
    # Observations — Phase 1: 20D base (+6 opponent) | Phase 2: +7
    # ------------------------------------------------------------------
    ...
    def get_observation_phase2(self, obs_phase1, allocation,
        ...
        Appends auction results + P5 emission shock + auction_savings
        + P15 coverage_ratio + carry_forward_norm.
        ...
        [base+5] coverage_ratio: holdings / obligation (clipped to [0, 3])  -- P15
                 Replaces coverage_penalty shaping: agent sees its compliance
                 position as information, learns to act on it naturally.
        [base+6] carry_forward_norm: carry_forward / max(emissions, 0.1)  -- P15
                 Normalized carry-forward burden; agents can see their rolled-over
                 shortfall relative to current emissions.
        """
        auction_savings = (allocation * 100.0 - payment) / 1000.0
        # P15: coverage ratio and carry-forward as observations (not shaping)
        obligation = max(emissions + self._carry_forward, 1e-6)
        holdings_total = banked + allocation
        coverage_ratio = min(holdings_total / obligation, 3.0)  # clip at 3 for stability
        carry_forward_norm = self._carry_forward / max(emissions, 0.1)
        ...
        extra = np.array([
            ...
            float(coverage_ratio),                                                  # [base+5] P15
            float(carry_forward_norm),                                              # [base+6] P15
        ], dtype=np.float32)
    ...
    def obs_dim_phase2(self) -> int:
        """obs_dim_phase1 + 7 (allocation, price, net_compliance_pos, emission_shock,
        auction_savings, coverage_ratio, carry_forward_norm)."""
