...
class ETSEnvironment(gym.Env):
    ...
    def _compute_rewards(self, payments, trade_costs, penalties,
        ...
        # P8→P9: Progressive banking holding cost parameters
        holding_cost_rate = trading_cfg.get("banking_holding_cost", 2.0)
        ...
        for i, company in enumerate(self.companies):
            ...
            # P9: Progressive banking holding cost — convex in bank/need ratio
            ...
            # get full credit, near-green agents get proportionally less.
            green_delta = max(0.0, company.green_frac - company.prev_green_frac)
            fossil_scale = max(company.fossil_frac, 0.05)  # floor to avoid zero
            shaping = beta_shaping * green_delta * fossil_scale * self.shaping_weight
            ...
            # P9: Bank value signal removed from shaping — conflicts with anti-hoarding.
            # Kept as diagnostic only (logged via _last_holding_costs).
            # bank_value = self.holdings[i] * clearing_price  # M-EUR (diagnostic only)
