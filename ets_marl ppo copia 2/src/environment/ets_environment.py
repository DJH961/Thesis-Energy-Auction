...
class ETSEnvironment(gym.Env):
    ...
    def step_auction(self, auction_actions: np.ndarray):
        ...
        # P15: Direct bid price — action[0] maps to [price_min, price_max].
        # No markup parameterization or reference anchor.
        # Agents must discover appropriate prices through learning.
        bid_actions[:, 0] = np.clip(bid_actions[:, 0], price_min, price_max)

        # Keep reference price for logging/diagnostics only (not used in bidding)
    ...
    def step_secondary(self, secondary_actions: np.ndarray):
        ...
        # Use realized emissions and existing carry-forward for compliance info.
        ...
        # P15: No sell_coverage_floor gating — agents sell freely.
        # Only constraint: no short selling (can't sell more than you hold).
        ...
        log.update({
            ...
            "max_safe_sell": pretrade_holdings.tolist(),  # P15: no sell gating; max = holdings
    ...
    def _compute_rewards(self, payments, trade_costs, penalties,
        ...
        """
        Reward (P15 merged):
          R_i = -(w_cost_i × cost_norm + w_green_i × emissions_intensity + penalty_norm)
                + green_bonus + queue_bonus

        Five signals:
          cost_norm:           total costs / 1000, weighted by per-agent w_cost
          emissions_intensity: penalisable EF / 0.82, weighted by per-agent w_green
          penalty_norm:        linear penalty (penalty / 100), always full weight (1.0)
          green_bonus:         diminishing-returns bonus (decays to zero with shaping_weight)
          queue_bonus:         construction queue reward (decays to zero with shaping_weight)

        Design: natural signals dominate (penalty/100 is 5× stronger than P13).
        Shaping (green + queue) provides early guidance but decays to zero
        (shaping_weight_floor=0.0), so agents ultimately learn from economics only.
        No coverage penalty — the 100 EUR/t penalty + carry-forward IS the signal.
        Holding cost excluded from reward (kept as diagnostic only).
        Per-agent w_cost/w_green weights create asymmetry: penalty is relatively
        stronger for cost-focused agents, discouraging floor bidding.
        ...
        for i, company in enumerate(self.companies):
            ...
            # Holding cost: computed for diagnostics only, NOT included in reward.
            # Agents learn banking strategy from natural penalty signals.
            holding_cost_rate = trading_cfg.get("banking_holding_cost", 0.0)
            holding_cost_exponent = trading_cfg.get("holding_cost_exponent", 1.5)
            annual_need = max(float(emissions[i]), 1e-6)
            bank_ratio = self.holdings[i] / annual_need
            if holding_cost_rate > 0 and bank_ratio > 1.0:
                excess = self.holdings[i] - annual_need
                progressive_rate = holding_cost_rate * (bank_ratio - 1.0) ** holding_cost_exponent
                holding_cost = excess * progressive_rate
            else:
                holding_cost = 0.0
            self._last_holding_costs[i] = holding_cost

            # Total cost excludes holding cost (diagnostic only)
            ...
            # P15: Stronger penalty signal — penalty / 100 (was / 500).
            # 100 M€ penalty → signal of 1.0 (was 0.2). 5× stronger gradient.
            # Always at full weight (1.0) — NOT scaled by w_cost/w_green.
            penalty_norm = (penalty_cost / 100.0) * non_compliance_mult

            # Green investment bonus with diminishing returns.
            # Decays to zero via shaping_weight (shaping_weight_floor=0.0).
            ...
            # Queue bonus — immediate reward for active construction projects.
            # Also decays to zero via shaping_weight.
            n_queue_active = sum(
                1 for item in company._construction_queue
                if item.get("frac_delta", 0) > 0
            )
            queue_bonus = gamma_queue * n_queue_active * 0.1 * self.shaping_weight

            # Per-agent w_cost/w_green weights on cost and emissions terms.
            # penalty_norm stays at full weight (1.0) for all agents.
            rewards[i] = float(
