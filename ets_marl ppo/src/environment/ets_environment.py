...
class ETSEnvironment(gym.Env):
    ...
    def _compute_rewards(self, payments, trade_costs, penalties,
        ...
        for i, company in enumerate(self.companies):
            ...
            # penalises overcoverage to prevent market cornering / hoarding.
            allocation_i = float(self._phase1_allocations[i])
            annual_emissions_i = max(float(emissions[i]), 0.01)
            raw_coverage = allocation_i / annual_emissions_i
            if raw_coverage <= 1.0:
                coverage_score = raw_coverage          # ramp up: 0 → 1
            elif raw_coverage <= 1.2:
                coverage_score = 1.0 - (raw_coverage - 1.0) * 5.0  # decay: 1 → 0
            else:
                coverage_score = -(raw_coverage - 1.2)  # negative penalty for hoarding
