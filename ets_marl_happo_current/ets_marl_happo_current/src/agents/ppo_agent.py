...
class PPOAgent:
    ...
    def __init__(self, agent_id, obs_dim_phase1, obs_dim_phase2,
        ...
        # Extra critic-only update epochs run before the joint actor+critic loop.
        # A better-fitted critic reduces advantage noise before the actor update.
    ...
    def _critic_loss(self, v_pred, ret_target, old_values=None):
        """
        Compute critic loss (Huber loss).

        Value clipping via torch.max(loss_unclipped, loss_clipped) was removed.
        When v_pred overshoots the clip range, v_clipped becomes a constant
        w.r.t. v_pred (clamp saturates), so loss_clipped has zero gradient.
        If torch.max selects loss_clipped, the critic receives no learning
        signal — exactly the opposite of what the trust-region was meant to do.
        This was the root cause of the failing test_separate_optimizers_step and
        of high/variable critic loss across agents.

        Huber loss already handles return outliers robustly (large penalty spikes,
        terminal values, stochastic shocks). The old_values parameter is kept for
    ...
    def update(self, last_value: float = 0.0, actor_update: bool = True) -> Optional[dict]:
        ...
        # ---------------------------------------------------------------
        # Critic-only pre-training epochs (critic_extra_epochs).
        # Running additional critic updates before the joint actor+critic
        # loop gives the value network more time to fit the current batch's
        # return targets.  Better V(s) estimates → lower-variance advantages
        # → cleaner actor gradient signal on this and subsequent updates.
        # The actor parameters are untouched during these epochs.
        # ---------------------------------------------------------------
        for _epoch in range(self.critic_extra_epochs):
            idx = np.arange(T)
        ...
        for _epoch in range(self.n_epochs):
            ...
            for start in range(0, T, self.mini_batch_size):
                ...
                # --- Critic step ---
                # retain_graph is NOT needed: actor and critic use separate
                # computation graphs (different network parameters, no shared
                # nodes). Removing it frees intermediate activations immediately.
                if not torch.isfinite(critic_loss_total):
                    continue
                self.critic_optimizer.zero_grad()
                critic_loss_total.backward()
    ...
    def update_happo(self, adv_t, ret_t, buf_tensors,
        ...
        if advantage_weights is not None:
            ...
            # Re-normalise after weighting.  The cumulative importance-ratio
            # product can shrink or inflate the advantage scale for agents
            # updated late in the HAPPO sequence (e.g. A8 receives near-zero
            # weights if an earlier agent made a large policy update).
            # Re-normalising restores a consistent N(0,1) scale so all agents
            # receive comparably strong gradient signals regardless of sequence
            # position.
        ...
        # ---------------------------------------------------------------
        # Critic-only pre-training epochs (critic_extra_epochs).
        # Same rationale as in update(): let the critic converge to the
        # current batch's return targets before the actor reads advantages.
        # ---------------------------------------------------------------
        for _epoch in range(self.critic_extra_epochs):
            idx = np.arange(T)
            np.random.shuffle(idx)
        ...
        for _epoch in range(self.n_epochs):
            ...
            for start in range(0, T, self.mini_batch_size):
                ...
                # --- Critic step ---
                # retain_graph not needed: actor and critic graphs are separate.
                if not torch.isfinite(critic_loss_total):
                    continue
                self.critic_optimizer.zero_grad()
                critic_loss_total.backward()
