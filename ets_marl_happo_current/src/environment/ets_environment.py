...
class ETSEnvironment(gym.Env):
    ...
    def _apply_warm_start(self, ws_cfg: dict):
        ...
        for i, company in enumerate(self.companies):
            ...
            # (B) Seed initial bank — scaled by emission intensity.
            # High-emitting companies (fossil_frac > 0.5) get a larger initial
            # bank, reflecting grandfathered free allocation proportional to
            # historical emissions in real EU ETS Phase I/II.
            annual_need = company.compute_emissions()  # Mt
            if company.fossil_frac > 0.5:
                seed_min_i = bank_min + 0.5   # higher floor for high-emitters
                seed_max_i = bank_max + 0.5   # higher ceiling too
            else:
                seed_min_i = bank_min
                seed_max_i = bank_max
            seed_multiple = float(self.rng.uniform(seed_min_i, seed_max_i))
    ...
    def _compute_rewards(self, payments, trade_costs, penalties,
        ...
        Reward (HAPPO-compliant):
          R_i = -cost_norm - emissions_intensity - penalty_norm
                + green_bonus + queue_bonus + coverage_signal

        Six core economic signals:
          ...
          coverage_signal:     immediate feedback on auction allocation vs estimated need
                               (bridges the credit-assignment gap between bid and compliance)
        ...
        coverage_signal_weight = reward_cfg.get("coverage_signal_weight", 0.3)
        ...
        for i, company in enumerate(self.companies):
            ...
            # Coverage signal: immediate feedback on auction allocation adequacy.
            # Bridges the credit-assignment gap between bid action and compliance
            # outcome. In real markets, firms track their coverage ratio (allowances
            # held vs obligation) as a key risk metric.
            ...
            if coverage_signal_weight > 0.0 and self._phase1_allocations is not None:
                ...
                need_i = max(float(emissions[i]) + float(
