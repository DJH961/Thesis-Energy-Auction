...
class ETSEnvironment(gym.Env):
    ...
    def __init__(self, config: dict, seed: Optional[int] = None):
        ...
        # LSTM bot policy (set externally by train.py if lstm_bots.enabled)
        self.bot_policy = None
        self._last_bot_obs_phase1 = None
        self._last_bot_obs_phase2 = None
        self._phase1_all_actions = None
        self._phase2_all_actions = None
    ...
    def reset(self, seed: Optional[int] = None, options=None):
        ...
        # Reset LSTM bot hidden states if active
        if self.bot_policy is not None:
            self.bot_policy.reset_hidden()
    ...
    def _generate_bot_auction_actions(self) -> np.ndarray:
        """Generate Phase-1 actions for all bot agents.
        Uses LSTM bot policy if available and ready, otherwise heuristic.
        Supports blending between LSTM and heuristic during transition.
        """
        if self.n_bots == 0:
            return np.zeros((0, 6), dtype=np.float32)

        # Always compute heuristic actions (needed for blending / fallback)
        ...
        heuristic_actions = np.zeros((self.n_bots, 6), dtype=np.float32)
        for b in range(self.n_bots):
            idx = self.n_agents + b
            heuristic_actions[b] = heuristic_policy.auction_action(
        ...
        # LSTM bot policy: blend with heuristic
        if self.bot_policy is not None and self.bot_policy.is_ready():
            alpha = self.bot_policy.compute_blend_alpha(
                self.current_episode, self.config)
            if alpha > 0.0:
                bot_obs = self._get_obs_phase1_for_bots()
                self._last_bot_obs_phase1 = bot_obs  # stash for data collection
                lstm_actions = self.bot_policy.auction_action(bot_obs)
                # NaN safety: fall back to heuristic for any NaN bot
                nan_mask = np.any(np.isnan(lstm_actions), axis=1)
                if np.any(nan_mask):
                    lstm_actions[nan_mask] = heuristic_actions[nan_mask]
                actions = (1.0 - alpha) * heuristic_actions + alpha * lstm_actions
                return actions
            else:
                self._last_bot_obs_phase1 = self._get_obs_phase1_for_bots()
        else:
            # Still build bot obs for data collection if bot_policy is attached
            if self.bot_policy is not None:
                self._last_bot_obs_phase1 = self._get_obs_phase1_for_bots()
    ...
    def _generate_bot_secondary_actions(self, clearing_price: float) -> np.ndarray:
        """Generate Phase-2 actions for all bot agents.
        Uses LSTM bot policy if available and ready, otherwise heuristic.
        """
        if self.n_bots == 0:
            return np.zeros((0, 2), dtype=np.float32)

        # Always compute heuristic actions
        heuristic_actions = np.zeros((self.n_bots, 2), dtype=np.float32)
        for b in range(self.n_bots):
            idx = self.n_agents + b
            heuristic_actions[b] = heuristic_policy.secondary_action(
                self.companies[idx],
                bank=float(self.holdings[idx]),
        ...
        if self.bot_policy is not None and self.bot_policy.is_ready():
            alpha = self.bot_policy.compute_blend_alpha(
                self.current_episode, self.config)
            if alpha > 0.0:
                bot_obs2 = self._last_bot_obs_phase2  # set in step_auction
                lstm_actions = self.bot_policy.secondary_action(bot_obs2)
                nan_mask = np.any(np.isnan(lstm_actions), axis=1)
                if np.any(nan_mask):
                    lstm_actions[nan_mask] = heuristic_actions[nan_mask]
                actions = (1.0 - alpha) * heuristic_actions + alpha * lstm_actions
    ...
    def step_auction(self, auction_actions: np.ndarray):
        ...
        # Stash full combined actions for LSTM bot data collection
        self._phase1_all_actions = auction_actions.copy()
        ...
        # Build bot Phase-2 observations for LSTM policy / data collection
        self._last_bot_obs_phase2 = None
        if self.bot_policy is not None and self.n_bots > 0:
            bot_obs1 = getattr(self, '_last_bot_obs_phase1', None)
            if bot_obs1 is not None:
                self._last_bot_obs_phase2 = self._get_obs_phase2_for_bots(
                    bot_obs1, allocations, clearing_price,
                    realized_emissions, epsilons, payments,
                )
    ...
    def step_secondary(self, secondary_actions: np.ndarray):
        ...
        # Stash full combined actions for LSTM bot data collection
        self._phase2_all_actions = secondary_actions.copy()
    ...
    def _get_obs_phase1_for_bots(self) -> np.ndarray:
        """Phase 1 observations for bot agents (indices n_agents .. n_total-1).
        Same structure as _get_obs_phase1 but for bot companies.
        Used by LSTM bot policy.
        """
        if self.n_bots == 0:
        ...
        for b in range(self.n_bots):
            ...
            obs_i = c.get_observation_phase1(
                ...
                last_clearing_price=self.last_clearing_price,
                expected_price=self.expected_price,
                auction_gap=self._last_gaps[idx],
                last_secondary_price=self.last_secondary_price,
                secondary_profit_signal=self._secondary_profit_ema[idx],
                price_ma3=price_ma3,
                opponent_obs=opponent_obs,
                last_secondary_volume=self.last_secondary_volume,
                tnac_proxy=tnac_proxy,
    ...
    def _get_obs_phase2_for_bots(
        self,
        bot_obs_phase1: np.ndarray,
        allocations: np.ndarray,
        clearing_price: float,
        realized_emissions: np.ndarray,
        epsilons: np.ndarray,
        payments: np.ndarray,
    ) -> np.ndarray:
        """Phase 2 observations for bot agents.
        Used by LSTM bot policy.
        """
        if self.n_bots == 0:
            return np.zeros((0, 0), dtype=np.float32)
        ...
        for b in range(self.n_bots):
            idx = self.n_agents + b
            obs_i = self.companies[idx].get_observation_phase2(
                obs_phase1=bot_obs_phase1[b],
                allocation=allocations[idx],
                clearing_price=clearing_price,
                emissions=realized_emissions[idx],
                banked=self.holdings[idx],
                emission_shock=float(epsilons[idx]),
                payment=float(payments[idx]),
            )
            obs_list.append(obs_i)
