...
def train_one_seed(config: dict, seed: int, on_log=None):
    ...
    # ── LSTM Co-Evolving Bots setup ────────────────────────────────────────
    lcd = config.get("lstm_bots", {})
    lstm_bots_enabled = lcd.get("enabled", False)
    lstm_bot_policy = None
    bot_buffer = None

    if lstm_bots_enabled and env.n_bots > 0:
        from src.agents.lstm_bot_policy import LSTMBotPolicy
        from src.agents.bot_data_buffer import BotDataBuffer

        obs1_dim = env.companies[0].obs_dim_phase1
        obs2_dim = env.companies[0].obs_dim_phase2

        lstm_bot_policy = LSTMBotPolicy(
            obs_dim_phase1=obs1_dim,
            obs_dim_phase2=obs2_dim,
            n_bots=env.n_bots,
            config=config,
        )
        bot_buffer = BotDataBuffer(
            capacity=lcd.get("buffer_capacity", 3000),
            n_total=env.n_total,
            n_years=n_years,
            obs_dim_phase1=obs1_dim,
            obs_dim_phase2=obs2_dim,
        )
        env.bot_policy = lstm_bot_policy

        lstm_retrain_interval = lcd.get("retrain_interval", 500)
        lstm_retrain_epochs = lcd.get("retrain_epochs", 3)
        lstm_retrain_batch = lcd.get("retrain_batch_size", 32)
        lstm_save_interval = lcd.get("save_interval", 2000)
        lstm_warmup = lcd.get("warmup_episodes", 0)
        if lstm_warmup <= 0:
            lstm_warmup = int(0.15 * n_episodes)

        print(f"LSTM bots: enabled | retrain every {lstm_retrain_interval} eps, "
              f"warmup={lstm_warmup}, buffer={lcd.get('buffer_capacity', 3000)}, "
              f"target={lcd.get('target_mode', 'imitate_best')}")
    ...
    for episode in range(n_episodes):
        ...
        # LSTM bots: begin episode data collection
        if lstm_bots_enabled and bot_buffer is not None:
            bot_buffer.begin_episode()
        ...
        for year in range(effective_n_years):
            ...
            # LSTM bots: collect year-step data for all participants
            if lstm_bots_enabled and bot_buffer is not None:
                bot_obs1 = getattr(env, '_last_bot_obs_phase1', None)
                bot_obs2 = getattr(env, '_last_bot_obs_phase2', None)
                if bot_obs1 is not None and bot_obs2 is not None:
                    obs1_all = np.concatenate([obs1, bot_obs1], axis=0)
                    obs2_all = np.concatenate([obs2, bot_obs2], axis=0)
                    auc_all = getattr(env, '_phase1_all_actions', np.zeros((env.n_total, 6)))
                    sec_all = getattr(env, '_phase2_all_actions', np.zeros((env.n_total, 2)))
                    bot_buffer.push_year(obs1_all, obs2_all, auc_all, sec_all)
        ...
        # LSTM bots: end episode data collection & periodic retraining
        if lstm_bots_enabled and bot_buffer is not None:
            # Build full reward vector for all participants
            yl_final = info.get("year_log", {})
            all_rewards_list = yl_final.get("rewards", [])
            if len(all_rewards_list) >= env.n_total:
                all_ep_rewards = np.array(all_rewards_list[:env.n_total], dtype=np.float32)
            else:
                # Fallback: use learning rewards, zero for bots
                all_ep_rewards = np.zeros(env.n_total, dtype=np.float32)
                all_ep_rewards[:n_agents] = total_rewards
            bot_buffer.end_episode(all_ep_rewards)

            # Periodic retraining (after warmup, data is sufficient)
            if (episode >= lstm_warmup
                    and episode > 0
                    and episode % lstm_retrain_interval == 0
                    and len(bot_buffer) >= lstm_retrain_batch):
                lstm_bot_policy.train_on_buffer(
                    bot_buffer,
                    epochs=lstm_retrain_epochs,
                    batch_size=lstm_retrain_batch,
                    verbose=(episode % (log_interval * 10) == 0),
                )

            # Periodic save
            if (lstm_save_interval > 0
                    and episode > 0
                    and episode % lstm_save_interval == 0):
                save_path = os.path.join(results_dir, f"lstm_bot_ep{episode}.pt")
                lstm_bot_policy.save(save_path)
        ...
        if episode % log_interval == 0:
            ...
            if n_bot_agents > 0:
                lstm_str = ""
                if lstm_bots_enabled and lstm_bot_policy is not None:
                    alpha = lstm_bot_policy.compute_blend_alpha(episode, config)
                    lstm_str = (f"  LSTM: α={alpha:.2f} "
                                f"retrain#{lstm_bot_policy.retrain_count}"
                                if lstm_bot_policy.is_ready() else "  LSTM: warmup")
