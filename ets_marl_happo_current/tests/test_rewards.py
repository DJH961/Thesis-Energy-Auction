...
def test_coverage_signal_rewards_adequate_allocation():
    """Agents with higher allocation coverage should get better coverage signal."""
    config = load_config()
    config["reward"]["coverage_signal_weight"] = 1.0  # amplify for test visibility
    config["simulation"]["n_years"] = 3
    config["warm_start"]["enabled"] = False
    config["uncertainty"]["enabled"] = False
    config["construction_jitter"]["enabled"] = False

    # High qty_mult → more allocation → better coverage signal
    env_high = ETSEnvironment(config, seed=42)
    env_high.reset()
    rewards_high, _ = _run_one_year(env_high, auction_price=100.0, qty_mult=1.3)

    config2 = load_config()
    config2["reward"]["coverage_signal_weight"] = 1.0
    config2["simulation"]["n_years"] = 3
    config2["warm_start"]["enabled"] = False
    config2["uncertainty"]["enabled"] = False
    config2["construction_jitter"]["enabled"] = False

    # Low qty_mult → less allocation → worse coverage signal
    env_low = ETSEnvironment(config2, seed=42)
    env_low.reset()
    rewards_low, _ = _run_one_year(env_low, auction_price=100.0, qty_mult=0.3)

    # Higher coverage should produce better (or equal) rewards
    assert rewards_high.mean() >= rewards_low.mean() - 0.1, (
        f"Higher coverage should improve rewards: high={rewards_high.mean():.3f}, "
        f"low={rewards_low.mean():.3f}")


def test_coverage_signal_zero_when_disabled():
    """With coverage_signal_weight=0, no coverage feedback."""
    config = load_config()
    config["reward"]["coverage_signal_weight"] = 0.0
    config["simulation"]["n_years"] = 3
    config["warm_start"]["enabled"] = False
    config["uncertainty"]["enabled"] = False
    config["construction_jitter"]["enabled"] = False
    ...
    env1 = ETSEnvironment(config, seed=42)
    env1.reset()
    rewards1, _ = _run_one_year(env1, auction_price=100.0, qty_mult=1.3)

    env2 = ETSEnvironment(config, seed=42)
    env2.reset()
    rewards2, _ = _run_one_year(env2, auction_price=100.0, qty_mult=1.3)
