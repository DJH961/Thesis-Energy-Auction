...
def test_obs_phase1_shape(config):
    """Phase 1 obs should be 25D base (no opponent modeling)."""
    c = make_company(config, agent_id=0)
...
def test_obs_phase1_with_opponents(config):
    """With opponent modeling, obs should have 25 + 5*(N-1) dims."""
...
def test_obs_phase2_extends_phase1(config):
    ...
    assert obs2.shape == (25 + 7,)
    # First 25 dims should match phase1
    np.testing.assert_array_equal(obs2[:25], obs1)
